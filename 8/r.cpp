...
void vyhod(linkedList &z) {
  //cout << "start" << endl;
  if(z.first == NULL) return;
  node* list_ptr = z.first;
  while((list_ptr->data == 0)&&(list_ptr->next != NULL)) {
    node* d = list_ptr;
    list_ptr = list_ptr->next;
    z.first = list_ptr;
    delete d;
  }
  if(z.first->next == NULL) {z.first = NULL; return;}
  while(list_ptr->next != NULL){
    if(list_ptr->next->data == 0) {
      //cout << "found 0" << endl;
      node* d = list_ptr->next;
      list_ptr->next = list_ptr->next->next;
      delete d;
    } else {
      //cout << "found " << list_ptr->next->data << endl;
      list_ptr = list_ptr->next;
    }
    //print(z);
  }
